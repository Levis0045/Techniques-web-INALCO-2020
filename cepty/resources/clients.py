...
__all__ = ['ClientsGetResource', 'ClientsUpdateDeleteResource',
            'ClientsLoginResource', 'ClientsPostResource']
...
class ClientsLoginResource(Resource):
    ...
    def get(self): 
        ...
        try:
            current_date = datetime.utcnow()+timedelta(minutes=360)
    ...
    def post(self):
        ...
        if 'password' not in crt_client: 
            try:
                client_id = crt_client['id']
                crt_client['password'] = data['password']
                results = update_data_by_id(client_id,'clients',crt_client,save=True)
            ...
            del crt_client['password']
            msg = {'data': crt_client, 'message': 'Password found! Login now', 'status': 409,
                    'time_server': datetime.now().isoformat(), 'total': 1}
            return output_json(msg, code=409)


class ClientsGetResource(Resource):
        ...
        if 'sort_name' in args:
            ...
            if len(database.keys()) >= 1:
                ...
                for c in database.values(): 
                    ...
                    results.append(c)
                results = sorted(results, key=lambda k: k['nom'])
            ...
            name = args['sort_type']
            len_data = len(database.keys())
            message = 'Successfully load clients by key'
            results = []
            for c in database.values(): 
                if 'password' in c: del c['password']
                results.append(c)
...
class ClientsPostResource(Resource):
    """ Get and create client(s)
